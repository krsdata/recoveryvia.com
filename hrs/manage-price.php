...
if ((isset($_POST['sub_multiform']) && $_POST['sub_multiform'] != "" && $_POST['sub_multiform'] == "delete"))
{
	...
	{
		$dataDel = $obj->getRowFromAnyTable('services_price','id',$_POST['checkbox'][$di]);
		@unlink('images/uploadUserImages/'.$dataDel['image']);
		$obj->deleteRowFromAnyTable('services_price','id',$_POST['checkbox'][$di]);
		$obj->deleteRowFromAnyTable('services_price','id',$_POST['checkbox'][$di]);
	}
	$obj->setMessageForNextPage('Record deleted successfully','MESSAGE_GREEN');
	header('location:'.REQUEST_URL);
...
{	
	...
	$obj->updateRowFromAnyTable("services_price",'status="'.$update.'"','id="'.$_GET['statusid'].'"');
...
if((isset($_POST['sub_multiform']) && $_POST['sub_multiform'] != "" && $_POST['sub_multiform'] == "status"))
{
	for($di=0;$di<count($_POST['checkbox']);$di++)
	{
		$dataStatus = $obj->getRowFromAnyTable('services_price','id',$_POST['checkbox'][$di]);
		$update= ($dataStatus['status'] == 1) ? 2 : 1;
		$obj->updateRowFromAnyTable('services_price','status="'.$update.'"','id="'.$_POST['checkbox'][$di].'"');
		$dataStatusUser = $obj->getRowFromAnyTable('services_price','id',$_POST['checkbox'][$di]);
...
{
 	$sql="SELECT  * from services_price  WHERE status=1 and service_name LIKE '%".$_GET['search']."%'";
 
} 
else
{
	$sql="SELECT  * from services_price where status=1  order by id DESC";
}
                ...
                <table border="0" width="100%" cellpadding="0" cellspacing="0" id="product-table">
                  <thead>
                    <tr>
                      ...
                      <th class="table-header-repeat line-left minwidth-1"><a onclick="SortTable(3);" href="javascript:;">Service Duration</a></th> 
                  ...
                  <tr <?php if($i%2==0){echo 'class="alternate-row"';}?>>
                    ...
                    <td><?php echo html_entity_decode($data['service_price']);?></td>
                  
                    <td class="options-width">
                    <a href="add-edit-price.php<?php echo DEF_QRY_STR;?>&editid=<?php echo $data['id'];?>&pagename=1" title="Edit" class="icon-1 info-tooltip">
